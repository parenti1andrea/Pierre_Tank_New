...
void Station::Inject(GroundParticle gp)
{
  ...
  if(strcmp(SampleMode,"SHOWER")) {
    ...
    if(!strcmp(SampleMode,"SNGL_FIX")){
      //printf("Launching particle with energy %f \n",gp.E);
      //printf("Position is %02f %02f %02f \n",gp.X, gp.Y, TANK_HEIGHT);
      //printf("cos(direction) is %02f %02f %02f \n",gp.CX, gp.CY, gp.CZ);
...
void Station::FollowMuon(ParticleInTank tpart)
{
  ...
  for(int i=0; i<MU_NSTEP; i++) {
    if(e_mu>MU_ESTEP[i]) {
      ...
      if(beta > 0.99999 and beta <=1){beta = 0.99999;};
...
#endif
          if(PrintTrace) {
            //printf("Number of slots per trace %d",PrintTrace);
            for(int k=0; k<NPMT; k++) {
              //for(int itr=0; itr<PrintTrace; itr++) fprintf(trace," %4d",stat.Pmt[k].HighGainTrace[itr]);
              for(int itr=0; itr<PrintTrace; itr++) fprintf(trace," %4f",stat.Pmt[k].HighGain[itr]);
	...
	// build FADC traces -------------------------------------------------------------
        ...
        if(PrintTrace) {
          ...
          for(int itr=0; itr<PrintTrace; itr++) {
            for(int k=0; k<NPMT; k++) fprintf(trace," %4d",stat->Pmt[k].HighGain[itr]);
            //for(int k=0; k<NPMT; k++) fprintf(trace," %4d",stat->Pmt[k].LowGainTrace[itr]);
            fprintf(trace,"\n");
	  }
        }
